...
class Auth
{
    ...
    public function login($email, $password, $remember = 0, $captcha = NULL)
    {
        ...
        if($sessiondata == false) {
            ...
            //Set the cookie
            setcookie($this->config->cookie_name,$sessiondata['hash'],$sessiondata['expiretime']);
        }
    ...
    public function logout($hash)
    {
        ...
        return $this->deleteSession($hash,true);
    }
    ...
    private function deleteSession($hash, $deleteCookie = false)
    {
    ...
    private function addRequest($uid, $email, $type, &$sendmail)
    {
        ...
        if($sendmail === true)
        {
            ...
            if($type == "activation") {
                ...
                $mail->Body = sprintf($this->lang['email_activation_body'], $this->config->site_url, $this->config->site_activation_page, $key,$key);
                $mail->AltBody = sprintf($this->lang['email_activation_altbody'], $this->config->site_url, $this->config->site_activation_page, $key,$key);
            }
            else {
